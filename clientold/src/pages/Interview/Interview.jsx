...
import { InterviewAnimation } from '../../components/ui/LottieAnim'
...
export default function Interview() {
  ...
  useEffect(() => {
    ...
    // Resume progress if this is the same interview session as before a refresh.
  ...
  // Keep progress in sync so a refresh resumes instead of restarting.
  useEffect(() => {
    if (!data?.interviewId) return
    localStorage.setItem(
      'interviewProgress',
      JSON.stringify({ interviewId: data.interviewId, current, scores })
    )
  }, [data, current, scores])
  ...
  const submitAnswer = async () => {
    ...
    try {
      ...
      setFeedback({
        feedback: 'Could not get AI feedback. Please check your API key.',
        score: 5,
        tip: 'Try again later.',
      })
      setScores([...scores, 5])
          ...
          <div className={styles.left}>
            ...
            <Card className={styles.answerCard}>
              ...
              <textarea
                ...
                rows={7}
                placeholder="Type your answer here... Be specific, use examples from your experience, and structure your response clearly."
                value={answer}
            ...
            {feedback && (
              <Card className={styles.feedbackCard}>
                <div className={styles.feedbackTop}>
                  <h3><FaRobot /> AI feedback</h3>
                  <div
                    className={styles.scoreChip}
                    style={{
                      color: getAnswerScoreColor(feedback.score),
                      borderColor: getAnswerScoreColor(feedback.score),
            ...
            <Card className={styles.sideCard}>
              ...
              <ul className={styles.qListSide}>
                {questions.map((q, i) => (
                  <li
                    key={i}
                    className={`${styles.qSideItem} ${i === current ? styles.qSideActive : ''} ${i < current ? styles.qSideDone : ''}`}
                  >
                    <span className={styles.qSideBadge}>{i < current ? <FaCheck /> : i + 1}</span>
                    <span>{q.length > 52 ? `${q.slice(0, 52)}...` : q}</span>
                  </li>
                ))}
              </ul>
            </Card>
                    ...
                    <div key={i} className={styles.scoreRow}>
                      ...
                      <div className={styles.scoreBarWrap}>
                        <div
                          ...
                          style={{ width: `${s * 10}%`, background: getAnswerScoreColor(s) }}
                        />
                      </div>
                      <span>{s}/10</span>
                    </div>
